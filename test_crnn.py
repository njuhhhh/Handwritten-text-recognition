...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk('./test_data'):
        ...
        for f in files:
            ...
            test_img_list.append(os.path.join(root, f))
            img_name_list.append(f)
    tmp_list=[]
    results=[]
    a = Test_CRNN()
    for i in range(len(test_img_list)):
        tmp_list.append(test_img_list[i])
        if len(tmp_list)==32:
            tmp_results = a.test_img(tmp_list)
            results=results+tmp_results
            tmp_results=[]
            tmp_list=[]
        if i==len(test_img_list)-1:
            while 1:
                tmp_list.append(test_img_list[i])
                if len(tmp_list)==32:
                    break
    ...
    result_file=open('char_test.txt','a')
    for i in range(len(img_name_list)):
        result_file.write(img_name_list[i]+' '+results[i]+'\n')
    result_file.close()
